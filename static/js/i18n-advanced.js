...
class vanilla_i18n {
  ...
  async run() {
    ...
    const attrsForTranslation = this._getAttrsForTranslation();
    this._translate_attrs(attrsForTranslation, langTranslations);
  }
  ...
  // translate all the attributes with set i18n data attribute
  _translate_attrs(elements, translation = undefined) {
    if (!translation) return;

    elements.forEach((element) => {
      this._i18nDataAttr_4attr.forEach((potentialAttrib) => {
  ...
  _getAttrsForTranslation() {
    if (this._i18nDataAttr_4attr) {
      return document.querySelectorAll(this._i18nDataAttr_4attr.map((cls) => `[${cls}]`).join(", ")); // Creates a CSS selector that gets each attrib
    } else {
      return [];
    }
  }
